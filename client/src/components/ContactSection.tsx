...
export function ContactSection() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Attempt to save to the Cloudflare database API endpoint
      try {
        ...
        console.warn("Failed to save to database - proceeding with mailto redirect", dbError);
      }

      const subject = encodeURIComponent(`Consultation Inquiry: ${formData.service || "General"}`);
      const body = encodeURIComponent(
        `Name: ${formData.name}
Email: ${formData.email}
Phone: ${formData.phone || "Not Provided"}
Service Requested: ${formData.service || "Not Selected"}

Message:
${formData.message}`
      );

      // Use a temporary <a> element for universal browser/mobile compatibility
      const mailtoUrl = `mailto:manisha@globalcoachforyou.com?subject=${subject}&body=${body}`;
      ...
      toast({
        ...
        description: "Your email client has been opened to send this message securely.",
      });
      ...
      toast({
        title: "Error",
        description: "Failed to open your email client. Please email us directly.",
        variant: "destructive",
      });
    } finally {
